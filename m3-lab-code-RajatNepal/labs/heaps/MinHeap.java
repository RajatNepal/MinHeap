...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void decrease(int loc) {
		...
		if((array[loc].getValue().compareTo((array[parent(loc)]).getValue()))<0){
			Decreaser<T> var = array[(parent(loc))];
			array[parent(loc)] = array[loc];
			array[loc] = var;
			array[loc].loc = loc;
			array[(parent(loc))].loc = ((parent(loc)));
			ticker.tick();
			decrease((parent(loc)));
		}
		ticker.tick();
	...
	private void heapify(int where) {
		...
		if (left(where) > size || array[left(where)] == null){
			
			return;
		}
		
		if (right(where) > size || array[right(where)] ==null) {
			if((array[left(where)].getValue().compareTo((array[where]).getValue())<0)){
				swap(left(where), where);
				ticker.tick();
			}
			return;
		}
		...
		if(array[left(where)] != null){
			if(array[right(where)] != null){
				if (array[left(where)].getValue().compareTo(array[where].getValue()) < 0
						&&	array[left(where)].getValue().compareTo(array[right(where)].getValue()) <= 0){
					swap(where, left(where));
					ticker.tick();
					heapify(left(where));
				}
				else if(array[right(where)].getValue().compareTo(array[where].getValue()) < 0) {
					swap(where, right(where));
					ticker.tick();
					heapify(right(where));
				}
			}		
